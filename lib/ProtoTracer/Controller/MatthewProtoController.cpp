#include "MatthewProtoController.h"

DMAMEM int dmaDisplayMemory[346 * 6];

MatthewProtoController::MatthewProtoController(CameraManager* cameras, uint8_t maxBrightness) : Controller(cameras, maxBrightness, 0), leds(OctoWS2811(ledsPerStrip, dmaDisplayMemory, drawingMemory, config)){
}

void MatthewProtoController::Initialize(){
    leds.begin();
    leds.show();
}

void MatthewProtoController::Display(){
    UpdateBrightness();
...
void MatthewProtoController::SetPixels(uint8_t strip, uint16_t led, RGBColor rgb){
    leds.setPixel(346 * strip + led, rgb.R, rgb.G, rgb.B);//Pin 8
}

void MatthewProtoController::SetBrightness(uint8_t maxBrightness){
...
void MatthewProtoController::SetAccentBrightness(uint8_t maxAccentBrightness){
